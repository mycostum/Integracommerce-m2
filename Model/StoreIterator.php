...
class StoreIterator implements StoreIteratorInterface
{
    ...
    /** @var \Mycostum\IntegraCommerce\StoreConfig\Context */
    protected $configContext;

    /** @var \Mycostum\IntegraCommerce\Integration\Context */
    protected $integrationContext;

    public function __construct(
        Context $context,
        \Mycostum\IntegraCommerce\StoreConfig\Context $configContext,
        \Mycostum\IntegraCommerce\Integration\Context $integrationContext,
    ...
    public function iterate($subject, $method, array $params = [])
    {
        ...
        $this->context->eventManager()->dispatch('mycostum_integracommerce_store_iterate_before', $eventParams);
        ...
        foreach ($this->getStores() as $store) {
            ...
            $this->context->eventManager()->dispatch('mycostum_integracommerce_store_iterate', $eventParams);

            $this->call($subject, $method, $params, $store);
        }

        $this->context->eventManager()->dispatch('mycostum_integracommerce_store_iterate_after', $eventParams);
    ...
    public function call($subject, $method, array $params = [], StoreInterface $store, $force = false)
    {
        ...
        try {
            ...
            $this->context
                ->eventManager()
                ->dispatch(
                    'mycostum_integracommerce_store_iterate_exception',
                    [
                        'exception' => $e,
                        'subject' => $subject,
                        'method' => $method,
    ...
    protected function initStores()
    {
        ...
        try {
            ...
            $this->context->eventManager()->dispatch('mycostum_integracommerce_store_init_stores', [
                'stores' => $stores,
            ]);
