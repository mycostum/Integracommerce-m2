...
namespace Mycostum\IntegraCommerce\Integration\Processor\Sales;

use Mycostum\IntegraCommerce\Api\OrderRepositoryInterface as IntegracommerceOrderRepositoryInterface;
...
use Mycostum\IntegraCommerce\Integration\Support\Sales\Order\CreateFactory as OrderCreatorFactory;
use Mycostum\IntegraCommerce\Helper\Sales\Order as OrderHelper;
use Mycostum\IntegraCommerce\Integration\Processor\Sales\Order\Status as StatusProcessor;
use Mycostum\IntegraCommerce\Api\Data\OrderInterfaceFactory;
use Mycostum\IntegraCommerce\Helper\Customer\Attribute\Mapping as CustomerAttributeMappingHelper;
...
class Order extends AbstractProcessor
{
    ...
    use \Mycostum\IntegraCommerce\Traits\Customer;

    /** @var string */
    const ADDRESS_TYPE_BILLING  = \Mycostum\IntegraCommerce\Integration\Support\Sales\Order\Create::ADDRESS_TYPE_BILLING;

    /** @var string */
    const ADDRESS_TYPE_SHIPPING = \Mycostum\IntegraCommerce\Integration\Support\Sales\Order\Create::ADDRESS_TYPE_SHIPPING;


    /** @var OrderRepositoryInterface */
    protected $orderRepository;

    /** @var IntegracommerceOrderRepositoryInterface */
     ...
     * @param IntegracommerceOrderRepositoryInterface $integracommerceOrderRepository
    ...
    public function __construct(
        ...
        IntegracommerceOrderRepositoryInterface $integracommerceOrderRepository,
    ...
    public function createOrder(array $data)
    {
        try {
            ...
            $this->eventManager()
                ->dispatch('mycostum_integracommerce_order_import_exception', [
                    'exception' => $e,
                    'order_data' => $data,
                ]);
    ...
    protected function processOrderCreation(array $data)
    {
        ...
        $status = $this->statusProcessor->getStateByIntegracommerceStatusType(
        ...
        $paymentMethod   = \Mycostum\IntegraCommerce\Model\Payment\Method\Standard::CODE;

        $shippingCarrier = \Mycostum\IntegraCommerce\Model\Shipping\Carrier\Standard::CODE;
        ...
        /** @var \Mycostum\IntegraCommerce\Integration\Support\Sales\Order\Create $creator */
        ...
        $creator->setOrderInfo($info)
            ...
            ->setComment(__('This order was automatically created by IntegraCommerce import process.'));

        $products = $this->getProducts((array) $this->arrayExtract($data, 'items'));
        if (empty($products)) {
            throw new Exception(__('The IntegraCommerce products cannot be matched with Magento products.'));
        }
    ...
    {
        $config = $this->helperContext()->scopeConfig()->getValue('mycostum_integracommerce/method_shipping/marketplaces');
        if (!$config) {
            return false;
        }
    ...
    /**
     * @param array      $integracommerceOrderData
     ...
     * @throws Exception
     */
    protected function updateOrderStatus(array $integracommerceOrderData, SalesOrder $order)
    {
        $integracommerceStatusCode = $this->arrayExtract($integracommerceOrderData, 'code');
        $integracommerceStatusType = $this->arrayExtract($integracommerceOrderData, 'status/type');

        $this->statusProcessor->processOrderStatus($integracommerceStatusCode, $integracommerceStatusType, $order);
    ...
    {
        if ($this->config->getValue('mycostum_integracommerce/customer_address/concat_complement')) {
     ...
     * @throws LocalizedException
     */
    protected function getOrderId($integracommerceCode, $storeId = null)
    {
        ...
        $orderId       = $orderResource->getOrderId($integracommerceCode, $this->getStore($storeId)->getId());
    ...
    protected function getOrderIncrementId($code = null)
    {
        ...
        $useDefaultIncrementId = $this->helperContext()
            ->configContext()
            ->general()
            ->getIntegraCommerceModuleConfig('use_default_increment_id', 'cron_sales_order_import');
