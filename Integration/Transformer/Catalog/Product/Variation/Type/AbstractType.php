...
use Mycostum\IntegraCommerce\Integration\Transformer\Catalog\AbstractProduct;
...
abstract class AbstractType extends AbstractProduct implements TypeInterface
{
    ...
    protected function addVariation(Product $product, ProductEntityInterface $interface)
    {
        ...
        $this->context->eventManager()->dispatch('bseller_integracommerce_product_variation_create_after', [
            'product'   => $product,
            'variation' => $variation,
        ]);
    ...
    protected function addPricesToProductVariation(Product $product, ProductEntityInterface\Variation $variation)
    {
        ...
        if (!empty($price)) {
            $variation->addSpecification($mappedPrice->getIntegracommerceCode(), (float) $price);
        }
        
        $specialPrice = $this->productHelper->extractProductSpecialPrice($product, $attributeSpecialPrice, $price);
        
        if (!empty($specialPrice)) {
            $variation->addSpecification($mappedSpecialPrice->getIntegracommerceCode(), (float) $specialPrice);
        }
